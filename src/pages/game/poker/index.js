...
const poker = () => {
    ...
    const result = matrix.map((matrixItem, i) => {
      return matrixItem.map((item) => {
        ...
        if (hadHiddenCard && hiddenCardIndex === length) {
          item.hiddenCard =
            hiddenCard[Math.floor(Math.random() * hiddenCard.length)];
          item.used = false;
        }
        return item;
      });
  ...
  function turnOver(data, travel = false) {
    if (
      historyStack.some(
        (item) => item.suit === data.suit && item.card === data.card
      ) &&
      !travel
    )
      return;
    ...
    setEnd(
      _matrixCards.every((matrixCard) =>
        matrixCard.every((item) => item.status === "front")
      )
    );
  ...
  function CardItem(props) {
    ...
    return (
      <div>
        ...
        {item.status === "covered" && (
          <div
            className="w-[4.9875rem] h-[7.7rem] bg-[#2d325a] rounded"
            onClick={() => turnOver(item)}
          ></div>
        )}
      </div>
    );
    ...
    <div className="p-4 w-[100vw] h-[100vh] flex justify-center">
      ...
      <div className="flex flex-row justify-around items-center fixed bottom-[5.4rem] left-[1rem] right-[1rem]">
        <span
          className={`w-[4.8rem] py-[0.2rem] text-center text-[#4975ee] border-[#4975ee] rounded-[1.2rem] border border-[#4975ee] cursor-pointer ${
            historyStack.length !== 0 && pointer >= 0
              ? "opacity-100"
              : "opacity-40"
          }`}
          onClick={() => {
        ...
        <span
          className={`w-[4.8rem] py-[0.2rem] text-center text-[#4975ee] border-[#4975ee] rounded-[1.2rem] border border-[#4975ee] cursor-pointer ${
            historyStack.length !== 0 && pointer < historyStack.length - 1
              ? "opacity-100"
              : "opacity-40"
          }`}
          onClick={() => {
      ...
      <ul className="flex flex-row justify-around items-center fixed bottom-[1rem] left-[1rem] right-[1rem] bg-[#262b4c] p-[0.4rem] rounded-[0.8rem] text-[#fff]">
        {Object.entries(frontCards).map((frontCard, i) => {
          ...
          return (
            <li key={i}>
              <span className="w-[4rem] text-[1.6rem] text-[#7182e9]">
                {key[0].toUpperCase()}
              </span>
              <span className="inline-block w-[2rem] text-right text-[2rem] font-bold">
                {value}
              </span>
            </li>
          );
